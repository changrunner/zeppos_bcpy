...
class Dataframe:
    ...
    def to_sqlserver_creating_instance(
            pandas_dataframe, sql_configuration, additional_static_data_dict=None, index=False,
            discover_data_type=False, use_existing=False, batch_size=10000,
            audit_date=datetime.utcnow(), csv_full_file_name=None):
        dataframe = Dataframe(sql_configuration)
        dataframe.to_sqlserver(pandas_dataframe, additional_static_data_dict, index, discover_data_type,
                               use_existing, batch_size, audit_date, csv_full_file_name)
        return dataframe

    def to_sqlserver(self, pandas_dataframe, additional_static_data_dict=None, index=False,
                     discover_data_type=False,  use_existing=False, batch_size=10000,
                     audit_date=datetime.utcnow(), csv_full_file_name=None):
        ...
        bcp_file_format = BcpFileFormat.create_bcp_format_file_instance_from_dataframe(
            self.pandas_dataframe, TempFile().temp_full_file_name)
    ...
    @staticmethod
    def _add_fields(pandas_dataframe, additional_static_data_dict, audit_date, csv_full_file_name):
        pandas_dataframe = Dataframe._add_additional_static_data(pandas_dataframe, additional_static_data_dict)
