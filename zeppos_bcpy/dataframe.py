...
class Dataframe:
    def __init__(self, pandas_dataframe, sql_table, index=False, discover_data_type=False,
                 use_existing=False, batch_size=10000):
        self.pandas_dataframe = pandas_dataframe
        DfCleaner.clean_column_names_in_place(self.pandas_dataframe)
    ...
    def to_sqlserver_creating_instance(
            pandas_dataframe, sql_table, index=False, discover_data_type=False, use_existing=False, batch_size=10000):
        dataframe = Dataframe(pandas_dataframe, sql_table, index, discover_data_type, use_existing, batch_size)
    ...
    def to_sqlserver(self):
        ...
        self.sql_table.create(self.pandas_dataframe.dtypes.to_dict(), self.use_existing)
        Bcp(self.sql_table, bcp_file_format, bcp_temp_csv_file, self.batch_size).execute()
